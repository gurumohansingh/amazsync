    ...
    async updateSalesMatrix() {
        ...
        const weekly = async function () {
            for (var i = 0; i < skus.length; i++) {
                try {
                    ...
                    var response = await sellingPartnerOperationsService.getOrderMetricsWeekly(marketPlace, sku['amz_sku'])
        ...
        const monthly = async function () {
            for (var i = 0; i < skus.length; i++) {
                ...
                try {
                    ...
                    var response = await sellingPartnerOperationsService.getOrderMetricsMonthly(marketPlace, sku['amz_sku']);
        ...
        const threemonthly = async function () {
            ...
            for (var i = 0; i < skus.length; i++) {
                ...
                try {
                    ...
                    var response = await sellingPartnerOperationsService.getOrderMetricsThreeMonths(marketPlace, sku['amz_sku']);
        ...
        for (var i = 0; i < feeskus.length; i++) {
            try {
                ...
                var sku = feeskus[i], currencyCode = "USD", updateMatrix = {}
                if (sku['market_place'].toUpperCase() == "CA") {
                    marketPlace = constant.MARKETPLACE_ID_CA
                    currencyCode = "CAD"
                }
                ...
                if (!isNaN(apiResponse)) {
                    ...
                    mysql.query(updateRestock, [{update_reason:apiResponse}, sku['amz_sku'], sku['market_place']]);
                 }
            }
            catch (e) {
                log.error(e);
