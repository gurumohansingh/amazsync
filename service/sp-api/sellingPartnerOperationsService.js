...
class sellingPartnerOperationsService {
    ...
    async getFeesEstimateBySKU(marketPlace, sku, amz_current_price, currencyCode) {
        var bodyParams = {
            FeesEstimateRequest: {
        ...
        var response = await sellingPartnerAPIService.callAPiWithFullPath('getMyFeesEstimates', 'products', `/products/fees/v0/listings/${encodeURIComponent(sku)}/feesEstimate`, "POST", bodyParams);
        try {
            if (response && response.FeesEstimateResult.FeesEstimate && response.FeesEstimateResult.FeesEstimate.TotalFeesEstimate) {
                return response.FeesEstimateResult.FeesEstimate.TotalFeesEstimate.Amount
            }
            else {
    ...
    async fetchShipmentByName(params) {
        var shipments = await sellingPartnerAPIService.callAPi('getShipments', 'fulfillmentInbound', params);
        return shipments;
    }
    ...
    async fetchShipmentItemsById(params, marketplaceId) {
        ...
        var shipments = await sellingPartnerAPIService.callAPi('getShipmentItemsByShipmentId', 'fulfillmentInbound', query, params);
        return shipments;
    }
    async fetchShipmentItemsByNextToken(params) {
        //queryParams=null,pathParams
        var shipments = await sellingPartnerAPIService.callAPi('getShipmentItems', 'fulfillmentInbound', params);
        return shipments;
    }

    async getReportDocumentId(reportType, marketplaceIds){
        var reportDocumentId = await sellingPartnerAPIService.getreportId(reportType, marketplaceIds);
