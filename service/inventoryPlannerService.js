...
const mysql = require('./mysql'),
    { savePurchaseOrder, getPurchaseOrder, deletePurchaseOrder, getVirtualPurchaseOrder, getVirtualShipmentById,
        updateVirtualShipmentById, removeVirtualShipmentById, updateShipmenStatus, getShipmentByShipmentId,getShipmentByName } = require("../util/sqlquery");
...
class inventoryPlannerService {
    ...
    async fetchData(user, reportId) {
        return new Promise((resolve, reject) => {
            ...
            var params = {
                ...
                LastUpdatedAfter: new Date(new Date().setDate(new Date().getDate() - 2)),
                LastUpdatedBefore: new Date(new Date().setDate(new Date().getDate() + 1))
            }
    ...
    async getConfig(user) {
        var config = await sellerSettings.getSetting(user, constant.INVENTORY_PLANNER);
        return new Promise((resolve, reject) => {
    ...
    fetchOrders(page, config) {
        return new Promise((resolve, reject) => {
            axios.get(`${constant.INVENTORY_PLANNER_SERVICE_URL}?limit=1000&page=${page}`, {
                method: 'get',
                headers: { 'Content-Type': 'application/json', 'Account': config.Account, 'Authorization': config.Authorization },
            }).then(data =>
                resolve(data.data)
            )
                .catch(error => {
    ...
    async fetchShipmentDetails(shipment) {
        return new Promise(async (resolve, reject) => {
            ...
            sellingPartnerOperationsService.fetchShipmentItemsById(params, constant.MARKETPLACE_ID_US).then((ids) => {
                ...
                var loadNext = function (token) {
                    ...
                    sellingPartnerOperationsService.fetchShipmentItemsByNextToken(params).then((nextids) => {
    ...
    async savePurchaseOrder(shipment, shipmentItems) {
        var checkExist = await mysql.query(getShipmentByShipmentId, shipment.ShipmentId);

        log.info(shipment.ShipmentId,shipment.ShipmentName);
        if (checkExist.length > 0) {
         mysql.query(updateShipmenStatus, [shipment.ShipmentStatus, shipment.ShipmentId]);
        } else {
            var sku = [], totalShipments = 0;
            shipmentItems.forEach((item) => {
                sku.push({ sku: item['SellerSKU'], masterSKU: null, sent: item['QuantityShipped'], casePackQuantity: item['QuantityInCase'] });
    ...
    async getPurchaseOrder() {
        return new Promise((resolve, reject) => {
            ...
            mysql.query(getPurchaseOrder, null)
                .then(async (shipments)=>{
                    
                    var products = await productrService.getFullProducts();
                    shipments.forEach(async (data) => {
                        var prepareItems = [];
                        var items = JSON.parse(data["items"]);
                        items.forEach(element => {
                            var foundProduct = products.filter(product => product.sellerSKU == element.sku)
    ...
    async getVirtualShipments(id) {
        var virtualShipments = await mysql.query(getVirtualPurchaseOrder, id);
        var products = await productrService.getFullProducts();
        try {
    ...
    async addVirtualShipment(username, items, totalSent, shipmentName, marketPlace, wareHouse) {

        var current = new Date();
        var data = {
            reference: shipmentName,
            remoteId: shipmentName + `-${current.getMonth() + 1}-${current.getDate()}-${current.getFullYear()}`,
            status: "Active",
            created_date: new Date(),
            items: items,
            total: totalSent,
        ...
        inventory.forEach(async function (item) {
            ...
            if (stock.length > 0) {
                ...
                await inventoryService.updateInventoryStockNoMaster([updateStock, sku, wareHouse]);
            }
        });
    ...
    async getShipmentByName(shipmentName) {
        var existShipment=await mysql.query(getShipmentByName, [shipmentName.toLowerCase()]);
