module.exports = {
    login: "select role,firstname,password from users where email=? and enable=1",
    ...
    addProductFromSync: "INSERT INTO products (amazonPrepInstructions,sellerId,user,itemName,sellerSKU,status,itemNote,dateAdded,amazonASIN,productId,productIdType,amzlive,imageUrl,imageHeight,imageWidth,masterSku,ismasterSku,itemNameLocal,isActive,isActiveLocal,isPartSKUOnly,UPC,UPCLocal,EAN,EANLocal,casePackQuantity,casePackUPC,packageWeight,packageDimensions,packageWeightLocal,dimensions,dimensionsLocal,amazonOversized,hazmat,itemNoteLocal,amazonFNSKU,lastUpdateFromAmazon,additionalPrepInstructions,suppliers,expirationDateRequired,reshippingCost,prepMaterialCost,prepLaborCost,tag,amazonSnL,kit,changeLog,history,targetDaysInWarehouse,targetDaysInAmazon,countryofOriginLocal,htcCodeLocal) VALUES?",
    updateProduct: 'update products set amzlive=1,amazonPrepInstructions=?,itemName=?,status=?,itemNote=?,productId=?,productIdType=? where sellerId=? and sellerSKU=?',
    ...
    getSuppliersByName: 'select supplierName from suppliersList where supplierName=?',
    ...
    productSuppliers: 'select s.*,sl.supplierName,sl.defaultTax from suppliers s  INNER join suppliersList sl on s.supplierID=sl.id where s.productSKU =?',
    addProductSuppliers: 'insert into suppliers SET ?',
    updateProductSuppliers: 'update suppliers SET ? where productSKU=? and supplierID=?',
    deleteProductSuppliers: 'delete from suppliers where productSKU=? and supplierID=?',
    getProductSuppliers: 'select id from suppliers where productSKU=? and supplierID=?',
    ...
    getBinLocationsbyName: 'SELECT * FROM binlocation WHERE warehouseId=? and name=?',
    validateBilocation: 'SELECT GROUP_CONCAT(sku) as sellerSKU from inventorystock where warehouseId=? and locationid=? and sku !=?',
    removeproductlocation: 'delete from inventorystock  where warehouseId=? and locationid=? and sku !=?',
    getAllKitProducts: 'SELECT imageUrl,sellerSKU,itemName,amazonASIN from products where kit != true and sellerSKU !=? and LOWER(sellerSKU) LIKE ? or LOWER(itemName) like ? or LOWER(amazonASIN) like ?',
    addkitproduct: 'INSERT INTO kitProducts SET ?',
    getKitProducts: 'SELECT p.imageUrl,kt.count,kt.sku as sellerSKU,p.itemName,p.amazonASIN from kitProducts kt inner join products p on kt.sku= p.sellerSKU where kt.parentSku=?',
    getKitProductsWithLocation: 'SELECT p.imageUrl,kt.count,kt.sku as sellerSKU,p.itemName,p.amazonASIN,location.stock as localCount,location.name as location from kitProducts kt inner join products p on kt.sku= p.sellerSKU  left join(select stock, inventorystock.sku, binlocation.name,masterSKU from inventorystock left JOIN binlocation on inventorystock.locationid = binlocation.id WHERE inventorystock.warehouseId = ?) as location ON location.sku = kt.sku where kt.parentSku = ?',
    ...
    updateInventoryStock: 'update inventorystock set stock=?,masterSKU=? where sku =? and warehouseId=?',
    updateInventoryStockNoMaster: 'update inventorystock set stock=? where sku =? and warehouseId=?',
    ...
    allUsers: 'select ID ,email,role,firstname,middlename,lastname,creationdate,enable from users',
    ...
    getVirtualPurchaseOrderCount: 'SELECT COUNT(*) as totalOrders FROM purchase_orders where is_virtual =?',
    ...
    removeVirtualShipmentById: "UPDATE purchase_orders set is_virtual=2,status=? WHERE id=?",
    updateShipmenStatus: "UPDATE purchase_orders set status=? WHERE remoteId=?",
    getShipmentByShipmentId: "select  status from purchase_orders WHERE remoteId=?",
